...
def decode_op_code(ops: str) -> OpProfile:
    ...
    if match:
        mS_val, mDe_val, func1, func2, animal1, animal2, animal3, animal4, social = match.groups()[:9]

        def get_coin_value(check1: bool, check2: bool, value1: str, value2: str) -> Optional[str]:
            return value1 if check1 else value2 if check2 else None

        op_profile = OpProfile(
            mS=get_coin_value(mS_val == MASCULINE, mS_val == FEMININE, MASCULINE, FEMININE) or None,
            mDe=get_coin_value(mDe_val == MASCULINE, mDe_val == FEMININE, MASCULINE, FEMININE) or None,
            ObserverDecider=OBSERVER if is_observer(func1) else DECIDER if is_decider(func1) else None,
            DiDe=get_coin_value(is_di(func1) or is_di(func2), is_de(func1) or is_de(func2), Di, De) or None,
            OiOe=get_coin_value(is_oi(func1) or is_oi(func2), is_oe(func1) or is_oe(func2), Oi, Oe) or None,
            NS=get_coin_value(is_sensing(func1) or is_sensing(func2), is_intuition(func1) or is_intuition(func2), SENSING, INTUITION) or None,
            FT=get_coin_value(is_thinking(func1) or is_thinking(func2), is_feeling(func1) or is_feeling(func2), THINKING, FEELING) or None,
            SleepPlay=get_coin_value(is_sleep(animal1) or is_sleep(animal2), is_play(animal1) or is_play(animal2), SLEEP, PLAY) or None,
            ConsumeBlast=get_coin_value(is_consume(animal1) or is_consume(animal2), is_blast(animal1) or is_blast(animal2), CONSUME, BLAST) or None,
            InfoEnergy=get_coin_value(is_info(animal4), is_energy(animal4), INFO, ENERGY) or None,
            IntroExtro=get_coin_value(is_intro(animal4), is_extro(animal4), INTRO, EXTRO) or None,
            FlexFriends=get_coin_value(is_flex(social), is_friends(social), FLEX, FRIENDS) or None,
            GenSpec=get_coin_value(is_spec(social), is_gen(social), SPECIALISATION, GENERALISATION) or None,
