...
class NeuralNetworkClassifier(ClassifierModel):
    ...
    def _evaluate(self, X, y, coin):
        ...
        for fold, (train_index, test_index) in enumerate(kf.split(X, y)):
            ...
            history = model.fit(X_train, y_train, epochs=10, batch_size=32, verbose=0, validation_data=(X_test, y_test), callbacks=[callback])
    ...
    def plot_history(self, histories, coin_name):
        ...
        for i, (label, history) in enumerate(histories.items()):
            ...
            axs[0].plot(history['val_loss'], linestyle='--', label=f'Validation Fold {i+1}')
            axs[1].plot(history['accuracy'], label=f'Train Fold {i+1}')
            axs[1].plot(history['val_accuracy'], linestyle='--', label=f'Validation Fold {i+1}')
