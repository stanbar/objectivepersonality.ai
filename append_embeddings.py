...
def ensure_embeddings(row):
    ...
    if pd.isna(row["embeddings"]):
        row["embeddings"] = calculate_embeddings(
            [row["transcript"]], row["transcript_tokens_length"]
        ).tolist()
    return row
